...
export default {
  ...
  methods: {
      ...
      addEntryData.forEach(async (val,idx)=>{
        if(val.notYetSaved) {
            ...
            if (res && res.status === 200 && res.data && res.data.data && res.data.data.length) { 
              ...
              "drcr": "Dr",   
              res.data.data[0]["drcr"] = 
              this.orginalTaggingtreeData.push(res.data.data[0])
            }    
        }
        else{
        let orgIndex = this.$_.findLastIndex(this.orginalTaggingtreeData, {
    ...
    updateSelection(e,val){
      console.log(e,val,"e",this.columnDefs,this.defaultColDef);
      let index = this.$_.findLastIndex(this.orginalTaggingtreeData, {
        name: val.name,
        parent: val.parent,
      });
      this.orginalTaggingtreeData[index]["selected"] = e;
    ...
    filterLevel1Options(e) {
      this.l1FilterOption = this.l1FilterOption.filter((val) => {
    ...
    async handleSelectParent(idx) {
      ...
      console.log(this.isParentDetailsAvailable,this.selectedParent )
